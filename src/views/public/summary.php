<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-20 relative z-10 text-center">
    <div class="w-24 h-24 rounded-none bg-success/10 border-4 border-success/20 flex items-center justify-center mx-auto mb-6 transform hover:scale-110 transition-transform duration-500">
    ...
    <div class="bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl rounded-none border border-gray-100 dark:border-gray-800 shadow-zp p-8 text-left max-w-xl mx-auto relative overflow-hidden">
        <!-- Decorative Header -->
        <div class="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-sport-400 to-sport-600"></div>
        <div class="absolute -right-10 -top-10 w-40 h-40 bg-sport-50 dark:bg-sport-900/20 rounded-none blur-2xl"></div>
        ...
        <div class="space-y-4 text-sm md:text-base relative z-10">
            ...
            <div class="flex justify-between items-start py-2 border-b border-gray-50 dark:border-gray-800/50">
                ...
                <span class="font-medium text-gray-900 dark:text-white text-right max-w-[60%]"><?= e($booking['notes']) ?></span>
            </div>
            <?php endif; ?>
            <div class="flex justify-between items-center py-2 border-b border-gray-50 dark:border-gray-800/50">
                <span class="text-gray-500 dark:text-gray-400">Waktu Main</span>
                <span class="font-medium text-gray-900 dark:text-white bg-gray-50 dark:bg-gray-800 px-3 py-1 rounded-none"><?= e(substr($booking['start_time'], 0, 5)) ?> - <?= sprintf('%02d:00', intval(substr($booking['start_time'], 0, 2)) + intval($booking['duration_hours'])) ?></span>
            </div>
            <div class="flex justify-between items-center py-2 border-b border-gray-50 dark:border-gray-800/50">
                ...
                ?>
                <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-none text-xs font-bold border <?= $label[1] ?>">
                    <?= $label[0] ?>
                </span>
            </div>
    ...
    <div class="mt-10 flex flex-col sm:flex-row items-center justify-center gap-4">
        ...
        <a href="<?= base_url('payment?id=' . $booking['id']) ?>" class="w-full sm:w-auto px-8 py-4 bg-gradient-to-r from-sport-500 to-sport-600 text-white text-base font-bold rounded-none hover:shadow-sport-500/40 transition-all duration-300 shadow-xl flex items-center justify-center gap-2 transform hover:-translate-y-1">
            <i data-lucide="upload" class="w-5 h-5"></i>
            Upload Bukti Pembayaran
        </a>
        <?php endif; ?>
        
        <a href="<?= base_url('home') ?>" class="w-full sm:w-auto px-8 py-4 bg-white dark:bg-gray-800 border-2 border-gray-200 dark:border-gray-700 text-gray-900 dark:text-white text-base font-bold rounded-none hover:bg-gray-50 dark:hover:bg-gray-700 transition-all duration-300 shadow-sm flex items-center justify-center gap-2 transform hover:-translate-y-1">
